# This python code use a .csv file with movies.
...
def random_movie_value():
    ...
    while True:
        ...
        if (
            (file["ratingAgeLimits"][random_value] == "r" or "age18")
            and (file["ratingKinopoiskVoteCount"][random_value] >= 2000)
            and (file["ratingKinopoisk"][random_value] >= 5.5)
            and (file["year"][random_value] >= 2000)
        ):
            break

        # Remove from bad quality movies.
        elif (
            (file["ratingKinopoiskVoteCount"][random_value] >= 1000)
            and (file["ratingKinopoisk"][random_value] >= 5.0)
            and (file["year"][random_value] >= 2000)
        ):
            break
    ...
    text_value = (
        f"{hbold(file['nameRu'][random_value])} "
