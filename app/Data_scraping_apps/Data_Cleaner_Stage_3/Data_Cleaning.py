...
df = df.drop(
    columns=["nameOriginal", "ratingMpaa", "ratingImdb", "imdbId", "shortDescription"]
)
try:
...
def filter_only_good_movies():
    ...
    df_v1_1 = df.drop(
        df[(df.rating_kinopoisk_vote_count < 3000) | (df.rating_kinopoisk < 7.2)].index
    )

    # Delete movies, which ARE NOT: "rating>=6.5" and "votes>=5000".
    df_v1_2 = df.drop(
        df[(df.rating_kinopoisk_vote_count < 5000) | (df.rating_kinopoisk < 6.5)].index
    )

    # Delete movies, which ARE NOT: "rating>=5.5" and "votes>=10000".
    df_v1_3 = df.drop(
        df[(df.rating_kinopoisk_vote_count < 10000) | (df.rating_kinopoisk < 5.5)].index
    )

    # Concatenate df_v1_1, df_v1_2 and df_v1_3 into a single dataframe.
    df_v2 = (
        pd.concat([df_v1_1, df_v1_2, df_v1_3]).drop_duplicates().reset_index(drop=True)
    )
    df_v2
    ...
    df_v2_1 = df_v2_1.drop(
        df_v2_1[
            (df_v2_1.rating_kinopoisk_vote_count < 15000)
            | (df_v2_1.rating_kinopoisk < 7.4)
        ].index
    )
    ...
    df_v2_2 = df_v2_2.drop(
        df_v2_2[
            (df_v2_2.rating_kinopoisk_vote_count < 10000)
            | (df_v2_2.rating_kinopoisk < 7.2)
        ].index
    )
    ...
    df_v3 = (
        pd.concat([df_v2_1, df_v2_2, df_v2_3]).drop_duplicates().reset_index(drop=True)
    )
    ...
    # Save all movies to a PostgreSQL database 'telegram_bot_db_all'.
    df_v3.to_sql(
        "telegram_bot_good_quality_movies_db", engine, if_exists="replace", index=False
    )
